...
int main(int argc, char **argv) {
    ...
    if ((g_datafiles = (char**)realloc(g_datafiles, 2 * sizeof(char *))) == NULL)
        unix_error("ERROR: realloc failed in main");
    strcpy(datadir, "./");
    g_datafiles[0] = const_cast<char*>("hahah");
    g_datafiles[1] = NULL;
    ...
    //            g_datafiles[num_datafiles - 1]);
    //     if (g_stats[num_datafiles - 1].valid) {
    //         printf(" => correct. \n");
    //     } else {
    //         printf(" => incorrect. \n");
    //     }
    ...
    //     printresults(num_datafiles, g_stats);
    //     printf("\n");
    // }
...
static void run_tests(int num_tracefiles, const char *tracedir,
    ...
    for (i = 0; i < num_tracefiles; ++i) {
        ...
        if (mm_stats[i].valid && error == 0) {
            printf("%s %d %.6f %.6f\n", flag,mm_stats[i].valid, 1 - mm_stats[i].util,
                   mm_stats[i].secs);
        } else {
...
static int add_range(range_t **g_ranges, char *lo, int size, const trace_t *trace,
                     int opnum, int index) {
    char *hi = lo + size - 1;
    range_t *p;
    assert(size > 0);
...
static trace_t *read_trace(stats_t *stats, const char *tracedir,
    ...
    if ((trace->block_rand_base =
             (int*)calloc(trace->num_ids, sizeof(*trace->block_rand_base))) == NULL)
    ...
    for (i = 0; i < trace->num_ops; i++) {
        ...
        switch (trace->ops[i].type) {
            case ALLOC:
                ...
                if (add_range(g_ranges, p, size, trace, i, index) == 0) return 0;
